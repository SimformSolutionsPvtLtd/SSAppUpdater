...
<update>
  ...
  <latestBuildURL>${build_url}${APP_NAME}_${version}.zip</latestBuildURL>
  <release_notes>
...
function compressApp() {
    ...
    zip -9 -y -q -r ${SCRIPTPATH}/AutoGeneratedContent/ExportedArchiveContent/"${APP_NAME}_${version}".zip "${APP_NAME}".app

    codesign --sign "Developer ID Application: SIMFORM LLC (K7XJG666ZW)" --options runtime --deep --force ${SCRIPTPATH}/AutoGeneratedContent/ExportedArchiveContent/"${APP_NAME}_${version}".zip "${APP_NAME}".app
